...
class network:
    ...
    def __init__(self, layers=[64 * 64, 100, 100, 62]):
...
for line in lines:
    elements = line.split(";")
    pixels = numpy.asarray(elements[1].split(","), dtype=float)
    pixel2d = numpy.reshape(pixels, (64, 64))
    pixel2d = numpy.transpose(pixel2d)
    plt.title(bytearray.fromhex(elements[0]).decode())
